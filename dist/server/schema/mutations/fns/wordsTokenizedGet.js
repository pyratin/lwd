...
var tokenizerGet = function tokenizerGet(tokenizerType) {
  switch (tokenizerType) {
    case 'wordPunct':
      return new _natural["default"].WordPunctTokenizer();

    default:
      return new _natural["default"].TreebankWordTokenizer();
  }
};

var _default = function _default(_sentence) {
  var tokenizerType = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : 'treebank';
  var tokenizer = tokenizerGet(tokenizerType);

  var sentence = _sentence.replace(/(\S)\/(\S)/g, '$1 / $2');

  var words = tokenizer.tokenize(sentence).reduce(function (memo, word, tokenIndex) {
