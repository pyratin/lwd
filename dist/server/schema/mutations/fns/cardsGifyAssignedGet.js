...
var queryGet = function queryGet(text) {
  return "\n    http://api.giphy.com/v1/gifs/search?api_key=".concat(process.env.npm_package_config_GIFY_API_KEY, "&limit=25&rating=pg-13&q=").concat(text, "\n  ").trim();
};
...
var cardsFlatlistGifyBase64AssignedGetFn = function cardsFlatlistGifyBase64AssignedGetFn(_ref2) {
  ...
  return (0, _nodeFetch["default"])(queryGet(encodeURIComponent(text))).then(function (res) {
    ...
    var randomIndex = Math.floor(Math.random() * json.data.length);
    var url = json.data.reduce(function (memo, _data, index) {
      if (!memo && index === randomIndex) {
        return _data.images['480w_still'].url;
      }
